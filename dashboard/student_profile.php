...
$stmt = $pdo->prepare('
    SELECT u.name, u.email, u.created_at, sp.roll_number, sp.class_id, c.class_name
    FROM users u
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if ($student_profile) {
                ...
                $stmt = $pdo->prepare('UPDATE student_profiles SET roll_number = ?, class_id = ?, bio = ?, phone = ? WHERE user_id = ?');
                $stmt->execute([$roll_number, $class_id, $bio, $phone, $user_id]);
            } else {
                // Insert student profile
                $stmt = $pdo->prepare('INSERT INTO student_profiles (user_id, roll_number, class_id, bio, phone) VALUES (?, ?, ?, ?, ?)');
                $stmt->execute([$user_id, $roll_number, $class_id, $bio, $phone]);
            }
            ...
            if ($roll_number !== $profile['roll_number']) $updated_fields[] = 'roll_number';
            if ($class_id != $profile['class_id']) $updated_fields[] = 'class';
            ...
            $stmt = $pdo->prepare('
                SELECT u.name, u.email, u.created_at, sp.roll_number, sp.class_id, c.class_name, sp.bio, sp.phone
                FROM users u
                LEFT JOIN student_profiles sp ON sp.user_id = u.id
                LEFT JOIN classes c ON sp.class_id = c.id
                WHERE u.id = ?
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-md overflow-hidden">
                        ...
                        <div class="px-6 pb-6 relative">
                            ...
                            <div class="text-center mt-4">
                                <h2 class="text-xl font-bold text-gray-900"><?php echo htmlspecialchars($profile['name'] ?? 'Student'); ?></h2>
                                ...
                                <div class="mt-4 space-y-2">
                                    <div class="flex items-center justify-center text-gray-600">
                                        ...
                                        <span><?php echo htmlspecialchars($profile['roll_number'] ?? 'Not set'); ?></span>
                                    </div>
                                    <div class="flex items-center justify-center text-gray-600">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                                        </svg>
                                        <span><?php echo htmlspecialchars($profile['class_name'] ?? 'Not assigned'); ?></span>
                                    </div>
                                ...
                                <div class="mt-4 bg-gray-50 rounded-lg p-4">
                                    ...
                                    <p class="text-gray-600 text-sm"><?php echo htmlspecialchars($profile['bio']); ?></p>
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-xl shadow-md p-6">
                        ...
                        <form action="student_profile.php" method="post" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label for="roll_number" class="block mb-2 font-medium text-gray-700">Roll Number</label>
                                    <input type="text" id="roll_number" name="roll_number" required 
                                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        value="<?php echo htmlspecialchars($profile['roll_number'] ?? ''); ?>" />
                                </div>
                                <div>
                                ...
                                <div>
                                    <label for="class_id" class="block mb-2 font-medium text-gray-700">Class</label>
                                    <select id="class_id" name="class_id" required 
                                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        <option value="">Select Class</option>
                                        <?php foreach ($classes as $class): ?>
                                            <option value="<?php echo $class['id']; ?>" 
                                                <?php if (($profile['class_id'] ?? '') == $class['id']) echo 'selected'; ?>>
